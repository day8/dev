...
(defn init! []
  (init-sci!)
  (let [root-elements   (seq (js/document.querySelectorAll ".cm-doc"))
        root-containers (mapv rdc/create-root root-elements)]
    (doseq [[^js el ^js container] (zipmap root-elements root-containers)
            :let                   [editable?     (not (.. el -dataset -cmDocNoEdit))
                                    ...
                                    state {:parent        el
                                           ...
                                           :!view         (atom nil)}]]
      (rdc/render container [editor state]))))
